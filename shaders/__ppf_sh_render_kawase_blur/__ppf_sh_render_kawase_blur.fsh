...
uniform float u_kawase_blur_mask_power;
uniform float u_kawase_blur_mask_scale;
uniform float u_kawase_blur_mask_smoothness;
uniform sampler2D u_kawase_blur_tex;
...
void main() {
	...
	vec4 col_blur = texture2D(u_kawase_blur_tex, uv);
	float mask = mask_radial(uv, vec2(0.5), u_kawase_blur_mask_power, u_kawase_blur_mask_scale, u_kawase_blur_mask_smoothness);
